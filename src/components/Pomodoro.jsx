...
export default function Pomodoro (){
    ...
    const [timer, setState] = useState({minute: 25, second: 0, text: '25:00', state: false});

    function startTimer(event){
        if(timer.state === false){
            console.log(timer.state);
            setState({ ...timer, state: true });
        }

        if(timer.state === true){
            console.log(timer.state);
            setState({ ...timer, state: false });
        }
    }
    ...
    return (
        <div className='pomodoro'>
            <div className='circle'>
                <span className='counter'>{ timer.text }</span>
